...
from PersonDetect import person_detect
from HelmetDetection import helmet_detect
...
def write_jsonFile(fileOutput,date_time):
    with open('./data/data'+date_time+'.json', 'w', encoding='utf-8') as f:
        json.dump(fileOutput, f, ensure_ascii=False, indent=4)
    return fileOutput

# delete file after Program end
def delete_file():
    dir = './person_crop'
...
def read_plate_detect():
    ...
    img_folder_path = './person_nohelmet'
    images_path = glob.glob("person_nohelmet/*.jpg")
    print(images_path)
    count = len(os.listdir(img_folder_path))

    # strip char Top Plate
    # txtFile = "stripTH.txt"
    # stripfile = open(txtFile,encoding="utf8")
    # strip_TH = stripfile.read()
    ...
    pts_ref = np.array([[0, 0], [509, 0], [0, 399], [509, 399], [254, 0], [509, 199], [254, 399], [0, 199],[20, 159], [489, 159], [20, 249], [489, 249], [20, 20], [489, 20], [20, 379], [489, 379]])
    ...
    for img_path in images_path:
        ...
        try:
            ...
            if len(rects) >= 1:
                ...
                print("T1.5")
                im_number_top, im_province, im_number_bottom = splitPlate(im_out)
                print("T2")
                # top
                ...
                # province
                result_province,maxProvScore = recognizeProvince(im_province, reader)
                result_province = result_province.strip()
                print("T3")
                # number
                result_number_bottom = recognizeNumber(im_number_bottom, reader)
                result_number_bottom = filter(str.isdigit,result_number_bottom) 
                result_number_bottom = "".join(result_number_bottom)
                
                print("TEST => ",result_number_top,result_province,result_number_bottom)

                # insert description plate ,array to json file
                plate_dict = {
                    "image":str(path_final_img_db)+'/'+str("person_"+str(z)+".jpg"),
                    "top":str(result_number_top), 
                    "province":str(result_province),
                    "bottom":str(result_number_bottom),
                    "score":str(maxProvScore)
                }
                result_OCRPlate.append(plate_dict)
                result_OCRplate_json = json.dumps(result_OCRPlate,ensure_ascii=False,indent=4).encode('utf8')
            ...
            if (int(person_Id) == int(count-1)):
                complete = False
                write_jsonFile(finaldata,date_time)
                # delete_file()
...
def startProgramDetection(pathUrl):
    ...
    # Person Detect,Helmet Detect Function
    person_detect(pathUrl)    
    helmet_detect()
    finaldata = read_plate_detect()
